...
Vfmax=12.0
Vfnstep=5
Vfg=np.linspace(Vfmin,Vfmax,Vfnstep)   #yoko3

for j,valfD in enumerate (Vfg):

	yoko3.ramp_voltage( valfD, 0.2, 0.01)

	Vmin=-bothBN/tophBN*valfD/divVc*divBg-8.0
	Vmax=-bothBN/tophBN*valfD/divVc*divBg+8.0
	Vnstep=13333
	Vg=np.linspace(Vmin,Vmax,Vnstep)   #yoko

	offset_start=0.5
	Vdmin=0.0+offset_start
	Vdmax=0.0015+offset_start   # good with div 10 on Vc
	Vdnstep=31
